...
class Elf:
    def __init__(self, cals: list[int]):
        self.cals: list[int]
        self.cals = cals
        self.max = max(self.cals)
        self.sum = sum(self.cals)
...
class Day1(DayChallenge):
    ...
    def run(self, input_data: Path) -> None:
        ...
        elves = Day1._convert_to_elves(data)
        elves.sort(reverse=True)

        # elf with most
        print("Part 1")
        print(f"The elf carrying the most carries {elves[0].sum} calories")

        # three elves with most
        print("\nPart 2")
        top_three = [elves[i].sum for i in range(3)]
        print(f"The three elves with the most calories carry {sum(top_three)}")

    @staticmethod
    def _convert_to_elves(data: list[str]) -> list[Elf]:
        elves: list[Elf] = list()
        cals: list[int] = list()
        items = data.copy()
        if not items[-1] == "":
        ...
        for item in items:
            if item == "" and len(cals) > 0:
                elves.append(Elf(cals=cals))
                cals.clear()
            else:
                cals.append(int(item))
        return elves
